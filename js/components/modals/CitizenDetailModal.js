                    ...
                    <div class="grid grid-cols-3 divide-x divide-white/5 h-fit">
                            ...
                            <div class="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-4">
                                ...
                                <div class="space-y-2.5">
                                    ...
                                    <div class="bg-white/[0.02] p-2.5 rounded-lg border border-white/5">
                                        ...
                                        <p class="text-sm text-white">{{ citizen.phone }}</p>
                                    </div>

                                    <div class="bg-white/[0.02] p-2.5 rounded-lg border border-white/5">
                                        <label class="text-sm text-white/30 uppercase font-bold tracking-wider block mb-0.5">Meslek</label>
                                        <p class="text-sm text-white">{{ citizen.job }}</p>
                                    </div>

                                    <div class="pt-1">
                                        <label class="text-sm text-white/30 uppercase font-bold tracking-wider block mb-1.5">Lisanslar</label>
                                        <div class="flex flex-wrap gap-1.5">
                                            <span v-for="license in citizen.licenses" :key="license" class="px-2 py-0.5 bg-sm-success/10 text-md-success border border-md-success/20 rounded text-[10px] font-bold uppercase">{{ license }}</span>
                                            <span v-if="!citizen.licenses || citizen.licenses.length === 0" class="text-sm text-white/30 italic">Kayıtlı lisans yok.</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="flex flex-col h-full bg-[#13131a]">
                            <div class="px-4 py-3 border-b border-white/5 bg-black/20 flex justify-between items-center">
                                <h4 class="text-sm font-bold text-white uppercase tracking-widest flex items-center">
                                    <svg class="w-4 h-4 mr-2 text-sm-danger" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/></svg>
                                    İlgili Davalar
                                </h4>
                                <span class="bg-md-danger/20 text-sm-danger text-[10px] font-bold px-1.5 py-0.5 rounded border border-md-danger/20">{{ citizen.criminalRecord ? citizen.criminalRecord.length : 0 }}</span>
                            </div>
                            <div class="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-2">
                                <div v-if="citizen.warrant" class="p-2.5 bg-md-danger/10 border border-md-danger/40 rounded-lg animate-pulse">
                                    <div class="flex items-center space-x-2 text-sm-danger mb-1">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                                        <span class="text-sm font-bold uppercase">Aktif Arama Kararı</span>
                                    </div>
                                ...
                                <div v-for="record in citizen.criminalRecord" :key="record.id"
                                    ...
                                    <div class="flex justify-between items-start mb-1.5">
                                        <span class="text-sm font-bold text-white group-hover:text-sm-danger transition-colors">{{ record.crime }}</span>
                                    ...
                                    <div class="pt-1.5 border-t border-white/5 flex justify-between items-center">
                                        <span class="text-sm text-white/30">Dosya No: #{{ 2023000 + record.id }}</span>
                                        <span class="text-sm text-white/30">Ofs. K. Wilson</span>
                                    </div>
                                </div>
                                ...
                                <div v-if="!citizen.criminalRecord || citizen.criminalRecord.length === 0" class="h-full flex flex-col items-center justify-center text-center p-8 opacity-50">
                                    ...
                                    <p class="text-sm font-bold text-white">Temiz Sicil</p>
                        ...
                        <div class="flex flex-col h-full bg-[#0f111a]">
                            <div class="px-4 py-3 border-b border-white/5 bg-black/20">
                                <h4 class="text-sm font-bold text-white uppercase tracking-widest flex items-center">
                                    <svg class="w-4 h-4 mr-2 text-sm-warning" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    Notlar & Varlıklar
                                </h4>
                            </div>
                                    ...
                                    <div class="space-y-1.5">
                                        <div v-if="citizen.notes && citizen.notes.length > 0">
                                            <div v-for="note in citizen.notes" :key="note.id"
                                                 :class="[
                                                    'p-2.5 border rounded-lg mb-1.5',
                                                    note.type === 'danger' ? 'bg-yellow-500/5 border-yellow-500/20' : 'bg-white/[0.03] border-white/5'
                                                 ]">
                                                <p :class="[
                                                    'text-sm leading-relaxed',
                                                    note.type === 'danger' ? 'text-white/80' : 'text-white/60'
                                                ]">"{{ note.text }}"</p>
                                                <div class="mt-1.5 flex justify-between items-center">
                                                    <span v-if="note.type === 'danger'" class="text-[10px] text-yellow-500/60 font-bold">⚠️ DİKKAT</span>
                                                    <span v-else></span>
                                                    <span :class="[
                                                        'text-[10px] font-mono',
                                                        note.type === 'danger' ? 'text-white/20' : 'text-white/20'
                                                    ]">{{ note.date }}</span>
                                                </div>
                                            </div>
                                        </div>
                                        <div v-else class="text-center py-4 opacity-30">
                                            <p class="text-[10px] text-white italic">Herhangi bir not bulunamadı.</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div class="space-y-1.5">
                                        <div v-if="citizen.assets && citizen.assets.length > 0">
                                            <div v-for="asset in citizen.assets" :key="asset.id" class="flex items-center justify-between p-2 bg-white/[0.03] rounded border border-white/5 mb-1.5">
                                                <div class="flex items-center space-x-2.5">
                                                    ...
                                                    <div>
                                                        <p class="text-sm font-bold text-white">{{ asset.model }}</p>
                                                        <p class="text-sm text-white/40 font-mono">{{ asset.plate }}</p>
                                                    </div>
                                                </div>
                                                <span :class="[
                                                    'w-1.5 h-1.5 rounded-full',
                                                    asset.status === 'danger' ? 'bg-md-danger animate-pulse' : 'bg-md-success'
                                                ]"></span>
                                            </div>
                                        </div>
                                        <div v-else class="text-center py-4 opacity-30">
                                            <p class="text-[10px] text-white italic">Kayıtlı araç bulunamadı.</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
